...
规则命中后不立刻发，等 20 秒重新抓一次，**两次都命中才推送**。挡掉 RPC 返回异常值、区块重组、瞬时抖动造成的假警报。

```
[confirm] 1 条命中，等 20s 后复检
[confirm] 复检完成 block 25798106 → 25798108
...
| `✓ 两次均命中` | 确认是真的，进入发送流程 |
| `✗ 复检未命中，判定为抖动` | 丢弃，不发送 |
| `~ 仅复检命中，留待下一轮确认` | 第二次才出现的，压住让它下轮走完整两次确认 |

两个实现细节：

- **必须放在抢占发送权之前**。否则第一次评估就占了 cooldown，最终没发的话这条报警在整个 cooldown 期内都报不出来了。
- **复检抓取失败时按首次结果发送**。RPC 抖一下不该让真报警被吞掉 —— 宁可误报，不可漏报。

设 `0` 关闭这个机制。代价是每次报警多 20 秒延迟，以及一次额外的 RPC 抓取。
