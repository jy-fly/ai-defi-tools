...
规则命中后不立刻发，隔一会儿再抓一次，**只有连续两次都命中才推送**。挡掉 RPC 返回异常值、区块重组、瞬时抖动造成的假警报。

```
[confirm] 2 条命中，等 20s 后复检
[confirm] 复检完成 block 25802074 → 25802076
[confirm] ✓ USDC-liq-drop-60m::USDC 两次均命中
[confirm] ✗ USDT-liq-drop-5m::USDT 复检未命中，判定为抖动，不发送
[confirm] 1 条仅复检命中，再等 10s 跑第三次
[confirm] ✓ WETH-liq-drop-10m::WETH 第二三次均命中
```

判定就是「连续两次」，用一个纯函数表达（`confirmedKeys`，7 个用例覆盖）：
...
第三次是为「只有复检才命中」的报警补一次确认。原来的做法是压住等下一个周期，但在 GitHub Actions 上下一个周期可能是半小时后，太久 —— 再等 10 秒就能定性，没必要拖。

三个实现细节：

- **确认必须在抢占发送权之前**。否则第一次评估就占了 cooldown，最终没发的话这条报警在整个 cooldown 期内都报不出来了。
- **复检抓取失败时按首次结果发送**。RPC 抖一下不该让真报警被吞掉 —— 宁可误报，不可漏报。
- **第三次抓取失败则留待下一轮**。这些报警只有一次命中记录，没有「连续两次」的证据，不该发。

`confirmDelaySeconds: 0` 关闭整个机制，`confirmRetrySeconds: 0` 只关第三次。代价是每次报警多 20-30 秒延迟和一到两次额外 RPC 抓取。
